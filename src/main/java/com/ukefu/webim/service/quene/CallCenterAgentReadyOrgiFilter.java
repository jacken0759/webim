...
public class CallCenterAgentReadyOrgiFilter implements KeyPredicate<String>{
	/**
	 * 
	 */
	 ...
	 * 
	 */
	public CallCenterAgentReadyOrgiFilter(String orgi){
		this.orgi = orgi ;
	}
	public boolean evaluate(String key) {
		CallCenterAgent agent = (CallCenterAgent) CacheHelper.getCallCenterAgentCacheBean().getCacheObject(key, orgi);
		return agent!=null && !StringUtils.isBlank(orgi) && orgi.equals(agent.getOrgi()) && agent.isReady();
	}
}