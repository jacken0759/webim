...
public class ClusterMasterListener implements MessageListener<Object>{
    ...
    public void onMessage(Message<Object> message) {
		...
		if(hazelcastInstance!=null && rpcDataBean!=null && !StringUtils.isBlank(rpcDataBean.getHost())) {
			ClusterContext.getInstance().setHost(message.getPublishingMember().getAddress().getHost());
			ClusterContext.getInstance().setPort(message.getPublishingMember().getAddress().getPort());
